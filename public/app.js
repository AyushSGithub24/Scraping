...
async function showData(data) {
  ...
  for (let ob of data.data) {
    ...
    li.addEventListener("click", async () => {
      ...
      // Show loading state
      div.innerHTML =
        '<div style="text-align: center; padding: 2rem; color: #6b7280;"><div class="loading" style="display: inline-block; margin-right: 0.5rem;"></div>Loading chapters...</div>';
      ...
      try {
        ...
        if (chapters.length > 0) {
          ...
          div.innerHTML =
            '<div style="text-align: center; padding: 2rem; color: #6b7280;">No chapters found for this comic.</div>';
        }
      } catch (error) {
        div.innerHTML =
          '<div style="text-align: center; padding: 2rem; color: #ef4444;">Error loading chapters. Please try again.</div>';
...
generateBtn.addEventListener("click", async () => {
  const selectedChapter = document.querySelector(
    '.chapter-list li[style*="border-color: rgb(102, 126, 234)"] > a'
  );
  ...
  try {
    const res = await fetch("http://localhost:3000/api/generate-video", {
      ...
      body: JSON.stringify({
        url: chapterURL,
        chapterNumber: selectedChapter.innerHTML,
      }),
    });
...
async function pollStatus(parentJobId) {
  try {
    const res = await fetch(
      `http://localhost:3000/api/job-status/${parentJobId}`
    );
    const status = await res.json();

    document.getElementById(
      "progress"
    ).innerText = `${status.progress}% - ${status.message}`;

    // If video is ready, show it
    if (status.step === "video-done" && status.url) {
      console.log(status)
      const video = document.getElementById("videoPlayer");
      video.src = status.url; // served by backend from /videos
      video.style.display = "block";

      setButtonLoading(false);
    } else {
      setTimeout(() => pollStatus(parentJobId), 2000);
    }
  } catch (err) {
    console.error("Polling error:", err);
    setTimeout(() => pollStatus(parentJobId), 3000); // retry after 3s if error
  }
}
